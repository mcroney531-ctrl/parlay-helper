...
type DataContextValue = {
  ...
  liveContextByIdeaId: Record<string, LiveContext>;
  loading: boolean;
...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const [liveContextByIdeaId, setLiveContextByIdeaId] = useState<Record<string, LiveContext>>({});
  ...
  const refreshLiveContext = useCallback(async () => {
    try {
      ...
      const byId: Record<string, LiveContext> = {};
      for (const ctx of all) byId[ctx.ideaId] = ctx;
      setLiveContextByIdeaId(byId);
    } catch (err) {
  ...
  const value = useMemo(
    ...
    [ideas, candidates, finalized, liveContextByIdeaId, loading, storageError, refreshIdeas, refreshCandidates, refreshFinalized, refreshLiveContext],
  );
