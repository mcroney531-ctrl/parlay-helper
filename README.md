...
- **Rate limiting**: `integrations/rateLimit.ts` caps `/api/odds` and `/api/events` (the two routes that spend `ODDS_API_KEY` quota) at a configurable requests/window per client key (defaults: 20 req / 60s, via `ODDS_RATE_LIMIT_MAX`/`ODDS_RATE_LIMIT_WINDOW_MS`), returning `429` with a `Retry-After` header past that. The tracked-key map is hard-capped (`MAX_TRACKED_KEYS`) with LRU-ish eviction, so cycling through many distinct identifiers can't grow it unboundedly.
  - **Client identity is untrusted by default.** `X-Forwarded-For`/`X-Real-Ip` are ordinary request headers a caller can set to anything — without a trusted proxy in front, trusting them would let an attacker rotate through fake values and bypass the per-client limit entirely. So by default every caller shares one bucket (more restrictive, never spoofable down to "no limit"). Only set `RATE_LIMIT_TRUST_PROXY=true` if this process genuinely sits behind a trusted reverse proxy/edge that appends the real client IP as the *last* hop of `X-Forwarded-For` (the standard single-trusted-hop convention) — verify that against your actual topology first; getting it wrong reopens the same spoofing hole.
  - **This is explicitly a second layer, not the primary control**, on a multi-instance/serverless deployment: it's an in-process `Map`, so each instance (Vercel function, etc.) holds its own counters, and a distributed attacker can multiply the effective limit by instance count. **If deploying this publicly with a real, quota-bearing `ODDS_API_KEY`, put platform/edge-level rate limiting in front of it** (e.g. a Vercel Firewall rate-limit rule, or your reverse proxy/CDN) — that's the control that actually holds under horizontal scaling. For a single-instance deployment or private/local testing, this in-repo layer is sufficient on its own.
